...
void ps2Handle() {                      //PS2 手柄 处理   
  ...
  if ( Mode == 0 )
  {
    if ( ps2X.Button( PSB_SELECT )   )
    {
      ...
      if (ps2X.ButtonPressed(PSB_GREEN)) {          //如果SELECT + 绿色按钮被按下
        LedFlip();
        FullActRun(13, 1);
        Timer = millis() + 50;
        return;
      }
      if (ps2X.ButtonPressed(PSB_BLUE)) {          //如果SELECT + 蓝色按钮被按下
        LedFlip();
        FullActRun(14, 1);
        Timer = millis() + 50;
        return;
      }
      if (ps2X.ButtonPressed(PSB_PINK)) {          //如果SELECT + 粉色按钮被按下
        LedFlip();
        FullActRun(15, 1);
        Timer = millis() + 50;
        return;
      }
      if (ps2X.ButtonPressed(PSB_RED)) {          //如果SELECT + 红色按钮被按下
        LedFlip();
        FullActRun(16, 1);
        Timer = millis() + 50;
        return;
